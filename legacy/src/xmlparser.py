...
class XMLParser:
    ...
    def __get_characters_speeches(self):
        ...
        def _extract_folger_text(elem):
            res = ""
            for child in elem:
                # ElementTree expands TEI tags to ``{namespace}local_name``.
                # Exact local-name matching matters here: ``fw`` is a running
                # page header, but ``endswith('w')`` used to mistake it for a
                # word and leak strings such as "ACT 1. SC. 4" into speeches.
                tag = child.tag.rsplit('}', 1)[-1]
                if tag in {'speaker', 'stage', 'sound', 'fw', 'head'}:
                    continue
                if tag == 'lb':
                    res += '\n'
                elif tag in {'w', 'c', 'pc'}:
