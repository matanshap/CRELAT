...
DEFAULT_CHRONOLOGICAL_BY_GENRE_FEATURES = [
    "y_mean", "y_std_of_averages", "y_average_of_std_devs",
]

FEATURE_LABELS = {
    "y_mean": "Mean BERT cosine similarity",
    "y_variance_of_averages": "Variance of pair-average BERT cosine similarity",
    "y_average_of_variances": "Average within-pair BERT cosine variance",
    "y_std_of_averages": "Std. dev. of pair-average BERT cosine similarity",
    "y_average_of_std_devs": "Average within-pair BERT cosine std. dev.",
    "y_root_average_of_variances": "Root average within-pair BERT cosine variance",
    "y_iqr": "IQR of pair-average BERT cosine similarity",
    "x_mean": "Mean interactions per character pair",
    "x_gini": "Gini of interactions per character pair",
    "x_top1_frac": "Top-pair interaction fraction",
    "pearson_r": "Pearson r: interactions vs BERT cosine",
    "isolation_dy_mean": "Mean y-distance from other plays",
    "isolation_xy_mean": "Retired: previous normalized year-y isolation",
}
...
def _compute_features(x_vals, y_vals, y_vars, y_mean_mode="pair"):
    ...
    if y_mean_mode == "interaction":
        ...
        y_average_of_std_devs = (
            float(np.average(np.sqrt(y_vars), weights=x_vals)) if total_x > 0 else np.nan
        )
    else:
        y_mean = float(np.mean(y_vals))
        y_average_of_variances = float(np.mean(y_vars))
        y_average_of_std_devs = float(np.mean(np.sqrt(y_vars)))
    ...
    features = {
        ...
        "y_average_of_std_devs": y_average_of_std_devs,
        "y_iqr": float(q75 - q25),
...
def _add_derived_features(records):
    """Populate cosine-unit variance features while preserving raw variances."""
    for r in records:
        r["y_std_of_averages"] = _sqrt_feature(r.get("y_variance_of_averages", np.nan))
        r["y_root_average_of_variances"] = _sqrt_feature(
            r.get("y_average_of_variances", np.nan)
        )
        if "y_average_of_std_devs" not in r:
            r["y_average_of_std_devs"] = r["y_root_average_of_variances"]
...
def compute_all_records(plays=PLAYS, top_n=TOP_N, min_cooc=MIN_COOC, model="bert", y_mean_mode="pair"):
    ...
    *model* can be any key from ``MODEL_REGISTRY`` (``"bert"``,
    ...
    _add_y_mean_isolation_feature(records)
    return records, all_interactions


def _mean_abs_y_distance(values):
...
def _add_y_mean_isolation_feature(records):
    """
    Keep the legacy y-mean isolation field, but measure only along y.

    The old normalized (year, y_mean) Euclidean metric is retired for this
    workflow because it mixes chronological position into the distance.
    """
    if not records:
        return
    yms = np.array([r.get("y_mean", np.nan) for r in records], dtype=float)
    mask = np.isfinite(yms)
    if mask.sum() < 2:
        for r in records:
            r["isolation_xy_mean"] = np.nan
            r["isolation_dy_mean"] = np.nan
        return

    v_yms = yms[mask]
    iso_dy = _mean_abs_y_distance(v_yms)
    idx = 0
...
def build_y_isolation_rows(records, features):
    rows = []
    for feat in features:
        indexed = []
        for i, r in enumerate(records):
            value = r.get(feat, np.nan)
            if np.isfinite(value):
                indexed.append((i, float(value)))
        if len(indexed) < 2:
            continue

        distances = _mean_abs_y_distance([value for _, value in indexed])
        for (record_idx, value), distance in zip(indexed, distances):
            ...
            rows.append({
                ...
                "year": r.get("year", ""),
                "value": value,
                "mean_abs_y_distance": float(distance),
            })
    rows.sort(key=lambda row: (row["feature"], row["mean_abs_y_distance"], row["title"]))
    return rows


def write_y_isolation_csv(records, features, output_dir, model_slug):
    rows = build_y_isolation_rows(records, features)
    path = os.path.join(output_dir, f"genre_y_isolation_{model_slug}.csv")
    os.makedirs(output_dir, exist_ok=True)
    with open(path, "w", encoding="utf-8", newline="") as f:
        writer = csv.DictWriter(
            f,
            fieldnames=[
                "feature", "title", "genre", "year", "value", "mean_abs_y_distance"
            ],
        )
        writer.writeheader()
...
def plot_chronological_by_genre(records, output_dir, features=None, degree=4):
    ...
    for i, feat in enumerate(features):
        ...
        if DecisionTreeClassifier is not None and len(all_yrs) >= 5:
            ...
            rules = _decision_tree_rules_text(dt, ["Year", feature_label], genre_set)
        ...
        if rax is not None:
            ...
            if rules:
                ...
                header = (
                    f"Decision tree (degree {degree}) accuracy for "
                    f"{FEATURE_LABELS.get(feat, feat)}: {acc_pct:.1f}% "
                    f"on {len(all_yrs)} plays"
                )
        ...
        ax.set_title(FEATURE_LABELS.get(feat, feat), fontsize=12, fontweight="bold")
...
def main():
    ...
    ap.add_argument("--features", type=str, default=None,
                    help="Comma-separated features for chronological_by_genre "
                         "(default: y_mean,y_std_of_averages,y_average_of_std_devs)")
    ...
    if "chronological_by_genre" in want:
        ...
        write_y_isolation_csv(records, feats, out, model_slug)
