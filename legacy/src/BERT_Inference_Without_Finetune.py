...
class BERTModelManager:
    ...
    def get_embeddings_batch(self, texts, batch_size=32, max_length=None):
        """
        ...
        for i in range(0, len(texts), batch_size):
            ...
            # Determine sequence limit from an explicit embedding-model config,
            # falling back to the transformer architecture's position limit.
            batch_max_length = max_length
            if batch_max_length is None:
                config = getattr(self.model, "config", None)
                if config is not None and not isinstance(config, dict):
                    batch_max_length = getattr(config, "max_position_embeddings", 512)
                elif isinstance(config, dict):
                    batch_max_length = config.get("max_position_embeddings", 512)
                else:
                ...
                if hasattr(self.model, "model") and hasattr(self.model.model, "block_size"):
                    batch_max_length = self.model.model.block_size
...
def get_embeddings_batch(
    ...
    return manager.get_embeddings_batch(
        texts,
